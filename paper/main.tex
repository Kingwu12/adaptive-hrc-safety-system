...
The concept-stage plan named four system measures (human--robot distance,
number of speed reductions and stops, number of yellow/red zone entries and
task completion time) and four feedback measures (safety, comfort,
predictability and trust), to be compared between static and adaptive safety.
Distance is reported directly (Table~\ref{tab:trials}, Figs.~\ref{fig:geometry}
and \ref{fig:trace}). Speed reductions and stops appear only as the
zero-request proportion per trial and the command trace, not as event counts,
because holds and proximity responses could not be separated in the recorded
stream. Zone entries were not counted, and capture span is reported instead of
task completion time because operator actions are inside the span. Safety and
trust are reported as intake/end ratings, comfort as the relaxed and calm
items, and predictability as the speed-legibility item. The static-versus-adaptive
comparison itself is the part that remains open, for the linkage and exposure
reasons given below.
