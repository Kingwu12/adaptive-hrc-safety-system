...
and trust) for comparison between static and adaptive safety. Distance is
reported directly (Table~\ref{tab:trials}, Figs.~\ref{fig:geometry} and
\ref{fig:trace}). Speed reductions and stops are represented by the
zero-request proportion and the command trace rather than by event counts,
because intentional holds and proximity responses cannot be separated in the
recorded stream. Zone entries were not counted, and capture span is reported
in place of task completion time because operator actions fall within the
span. Safety and trust are reported as intake/end ratings, comfort through the
