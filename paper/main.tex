...
\begin{document}
\title{Evaluating Safety and Coordination in Context-Aware Speed and Separation
Monitoring for Human--Robot Ceiling-Panel Installation}
\author{Zenan Wu, Luke Siniakov, and Michael Magila%
\thanks{Department of Civil Engineering, Monash University, Melbourne, Australia.
2026 Final Year Project under the primary supervision of Yihai Fang.}
}
\maketitle
\begin{abstract}
...
This paper presents a context-aware speed and separation monitoring prototype
that combines geometric constraints, task-phase recognition and short-horizon
intrusion prediction. A three-state Gaussian-mixture hidden Markov model supplies
task context, while a deterministic command ceiling prevents that context from
increasing the speed permitted by the reactive envelope in SSM mode. Development
validation across three operators achieved causal phase accuracy of 0.794 and
balanced accuracy of 0.725. An exploratory evaluation analysed fourteen telemetry
trials and 31 block-questionnaire submissions. Responses generally indicated
high stopping confidence and low perceived workload; eleven matched intake/end
records showed mean safety ratings of \SafetyPreMean{} before and
\SafetyPostMean{} after the session. Telemetry identified sensitivity to geometric
...
position, using an overhead safety camera, safety PLC and robot controller in an
automotive-inspired cell. These studies establish that adaptation and productivity
...
\subsection{Robot, Sensing and Geometry Versions}
The laboratory prototype uses a UR10 robot and a lightweight ceiling-panel
surrogate. OptiTrack supplies a head rigid-body position; the Xsens path records
23 segment positions and orientations. Robot telemetry supplies TCP pose and
joint velocities. The nominal dashboard loop is 60~Hz, although actual intervals
vary. The trial streams, manifests and event journals retain the observed timing.

For a tracked point $\mathbf p$ and a nearest point $\mathbf q$ on a vertical
column proxy, the head feature is
\begin{equation}
d=\|\mathbf q-\mathbf p\|_2,\qquad
v_{\rm proj}=\mathbf v^{\mathsf T}\mathbf u,
\end{equation}
where $\mathbf u$ points toward the proxy and positive projected velocity means
closing motion. A recent-sample slope estimates velocity; heading alignment is
velocity alignment, not torso orientation.

The analysed trials contain two geometry implementations. P17 and P22 use
head features that are not arithmetically consistent with a same-row live-TCP
column reconstruction. P24 records a live TCP reference for the head feature and
...
\end{equation}
The online forward filter uses present and past observations; offline Viterbi
...
\end{equation}
for a non-negative time within a 0.5-s horizon. The configured closing-speed gate
...
\end{equation}
with configured $T=0.4$~s, $C=0.2$~m and $S_a=0.1$~m. The ramp above $S$ is
...
\label{eq:bound}
\end{equation}
with a fixed inner-boundary override and conservative handling of unavailable
...
The phase classifier was developed using 32 accepted labelled trials from P03
(eight), P04 (twelve) and P05 (twelve), containing 90,487 frames. Class counts are
...
Offline accuracy is 0.821 and causal accuracy is 0.794
...
sequences and consistent participant linkage were unavailable. The analysis
included 14 telemetry trials associated with three study IDs and 31 block
questionnaire submissions associated with eleven IDs. Nine IDs had all three
block questionnaires and two lacked the final block response. Matched intake
and end responses were available for all eleven follow-up IDs. These counts
...
met these criteria. One additional trial record lacked its sample stream and
...
for time integrals. Forty rows carry a stale-tracking flag. Controller decisions
exist in \DecisionRows{} rows. One hundred and ten decisions indicate unavailable tracking and invoke the
conservative controller state. These counts characterise the sampled command
...
\caption{Measurement diagnostics. (a) Head features differ from the
same-row live-TCP column calculation. (b) Independent extrema within cue windows;
dashed lines show the configured 0.94-m boundary and 0.6-m/s closing gate. Colours
identify fixed (blue), reactive (orange) and predictive (green) records. Geometry
versions differ and extrema are not necessarily simultaneous.}
\label{fig:geometry}
\end{figure*}
...
and frustration (Table~\ref{tab:survey}, Fig.~\ref{fig:questionnaires}). Median
...
\begin{figure*}[t]
\centering\includegraphics[width=.94\textwidth]{figures/questionnaire-distributions.pdf}
\caption{Original item distributions across 31 block submissions. Each bar sums
to 31; embedded values are response counts. Higher scores mean more relaxed/calm,
more surprised, stronger agreement, or greater demand/frustration according to
the item. Pooling describes the recorded responses and does not rank controllers.}
\label{fig:questionnaires}
\end{figure*}

Across eleven matched records, mean safety ratings were \SafetyPreMean{} before
and \SafetyPostMean{} after the session. Eight ratings increased, one was
...
suit restriction is two (interquartile range 1.5--3). No risk or discomfort comments were entered in the eleven end forms; these
...
0.794 accuracy in development validation. An exploratory analysis of fourteen
