...
The launcher and dashboard detect older backends that still require manual recording steps. After deploying this code on the Central lab PC, restart with `Start-Lab.cmd`. This change has been tested locally with simulated hardware; it has not been deployed to the offline lab PC.
